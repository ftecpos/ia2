...
$sql3 = "select *, sum(qty*price)-sum(qty*cost)-discount as profit, sum(qty) as tol_qty from invoicedetail left join invoice on invoicedetail.invoice_no = invoice.invoice_no where invoice.invoiceState_no != 2";
...
//if($count3 == 0){
	while($row3 = mysql_fetch_assoc($result3)){
		...
		if($row3['goodsType'] == '1'){
			...
			if($row3['invoiceType_no'] == '2'){
				$total1 = ($row3['tol_qty']*$row3['price'])-(0-$row3['discount']);
			}else{	
			...
			if($row3['invoiceType_no'] == '2'){
				...
				$temp_discount = ($row3['discount']/$temp_total)*$row3['tol_qty'];
			...
			$total2 = ($row3['tol_qty']*$row3['price'])-$discount;
