...
<?php 
	...
	if($phone_IMEI == NULL){
		...
		$phone_sql = mysql_query("select phonetype_id, phone_name, description from phone ph, phonetype pt where ph.phoneType_no = pt.phoneType_no and ph.IMEI='".$phone_IMEI."'");
		echo "<tr>";
		echo '<td width="200" id="code"><div align="left">'.mysql_result($phone_sql,0,0).'</div></td>';
		echo '<td width="600" id="name"><div align="left">'.mysql_result($phone_sql,0,1).'<br>'.mysql_result($phone_sql,0,2).'</div></td>';
		echo '<td width="200" id="qty"><div align="center">'.mysql_result($list_sql,$i,2).'</div></td>';
		echo "</tr>";
	}else{
	echo "<tr>";
