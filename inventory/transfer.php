						...
						<table border="0">
								...
								<tr>
    								...
    								<td>
                        				...
                        				<?php
											$result = mysql_query("select * from retailshop where retailShop_no=".$_SESSION['retail_no']); 
					...
					<tr>
			    		...
			    		<td>傳貨至
                     		<select id ="branchto" name="branchto" onchange="disableSubmit(false)">
    							<option value=""></option>
                       			 <?php
									$result = mysql_query("select * from retailshop where retailshop_no !=9999 order by retail_id "); 
									while($row = mysql_fetch_array($result)) {
...
<script type="text/javascript">
function add_trans_msg(retailNo,print_trans_no){
	var msg = '/pm '+retailNo+' Transfer to : '+retailNo+' TransNo : '+print_trans_no;
		socket.emit('chat', msg);
}


	$('#printTrans').dialog('destroy');
	var dialogOption51 = {
	...
	$('#printTrans').dialog(dialogOption51);
//---End of printTrans dialog------------------------------
	
</script>
<!--// printTrans 張form-->
<div id="printTrans" title="列印轉貨單">
