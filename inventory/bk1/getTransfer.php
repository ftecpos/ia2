									...
									while($row = mysql_fetch_array($result)) {
										if($totalqty != 0) {
											if($row['ava_bal'] - $totalqty >= 0) {
												...
												mysql_query($sql2);
												$sql3 = "update stockin set ava_bal = ava_bal - ".$totalqty." where retailShop_no = ".$_SESSION['retail_no']." and acc_no = ".(checkAssNo($trandetaila[$i][0]))." and po_date = '".$row['po_date']."' and stockIn_no = ".$row['stockIn_no']; 
												mysql_query($sql3);
												$totalqty = 0;
												break;
											} else {
												...
												mysql_query($sql2);
												$sql3 = "update stockin set ava_bal = 0 where retailShop_no = ".$_SESSION['retail_no']." and acc_no = ".(checkAssNo($trandetaila[$i][0]))." and po_date = '".$row['po_date']."' and stockIn_no = ".$row['stockIn_no']; 
												mysql_query($sql3);
											}
										}
									}
				...
				if($row['IMEI'] == "")  {
					...
					while($row7 = mysql_fetch_array($result7)) {
						if($row7['ava_bal'] + $row['trans_qty'] <= $row7['rec_qty']) {
							$sql2 = "update stockin set ava_bal = ava_bal + ".$row['trans_qty']." where acc_no = ".$row['acc_no']." and po_date = '".$row['po_date']."' and retailShop_no = ".$row['fromRetail_no']." and stockIn_no = ".$row7['stockIn_no'];
						}
					}
				} else {
...
function checkEdit($state,$no,$staffno) {
	...
	if($state != "1") {
		...
		$out .= "<table border=0 id=editTable width=100%><tbody><tr><td>轉貨單編號</td><td colspan=1><input type=text id=editTranNo name=editTranNo size=10 disabled value=".$no."></td></tr>";
...
function checkConfirm($state,$no,$staffno) {
	...
	if($state != "1") {
		...
		$out .= "<table border=0 id=editTable width=100%><tbody><tr><td>轉貨單編號</td><td colspan=1><input type=text id=confirmTranNo name=confirmTranNo size=10 disabled value=".$no."></td></tr>";
