...
//echo $sql;
	...
	case 'getSupplierList':
		if ($_SESSION['retail_no']==1){
			$sql = "select supplier_no, supplier_id, supplierName, remark from supplier";
			$result = $db->query($sql);
			echo '<table id="" class="" border="1" width="100%" cellpadding="3">
					<tr><td td style="text-align:center">Supplier</td><td td style="text-align:center">Supplier Name</td><td td style="text-align:center">Remark</td></tr>';
			while ($row = $db->fetch_array($result)) {
				echo '<tr><td>';
				echo	'<a href="#" style="color:#0019FF;font-size:15px;" onclick="addSupplierToInvoice(\''.$row['supplier_id'].'\')">'.$row['supplier_id'].'</a>';
				echo'</td><td>'.$row['supplierName'].'</td><td style="text-align:center">'.$row['remark'].'</tr>';
			}
		}
		break;
			...
			$RESULT2=$DATE2_time-$DATE1_time;
			 ...
			 while ($row = $db->fetch_array($result)) {
				$retuenQty=array();
				 ...
				 //$smallTotal = ($row['price']-$row['discount'])*$row['qty'];
				 //$smallTotal = ($row['price']-$row['discount'])*$row['qty'];
...
$result = $db->selectLimit($sql, '14',$pageNo);
	...
	//join 左stockin			
		...
		if (isset($_GET['shortByA'])&& isset($_GET['shortByB'])){
			$shortByA = $_GET['shortByA'];
                        $shortByB = htmlentities($_GET['shortByB']);
			$sql .=" AND $shortByA like '%$shortByB%'
...
//		$pageNo = 0;
		...
		echo '<table rules="all" border="1" class="finAcc" style="width:100%;" >'.
            '<thead>'.
            	...
            	'<th style="width: 145px">配件名稱</th>'.
            	'<th style="width: 100px">配件種類</th>'.
            	'<th style="width: 100px">配件顔vv色</th>'.
				'<th>oprice</th>'.
            	'<th>sprice</th>'.
            	'<th>配件狀態</th>'.
				'<th>店舖</th>'.
		...
		if ($result) {
                while ($row = $db->fetch_array($result)) {
					echo '<tr><td><a href="#" style="color:#0019FF;"
					 onclick="addAccToTransfer('.$row['acc_no'].')">'.$row['acc_no'].' 加到轉貨單</a></td>'.
            				     '<td>'.$row['acc_id'].'</td>'.
            				     '<td>'.$row['manufacturer'].'</td>'.
            				     '<td>'.$row['accName'].'</td>'.
            				     '<td>'.$row['typeName'].'</td>'.
...
//		$pageNo = 0;
	...
	case 'salesReturn':
            ...
            $sql ="SELECT invD.*, inv.retailShop_no, inv.createBy
                   FROM invoicedetail invD, invoice inv
...
//		$qty =(isset($_GET['returnQty']))? $_GET['returnQty'] :$row['qty'];
            ...
            //$invoice_create_retailShopNo=$_GET['retailShopNo'];
            $retailShopNo=$row['retailShop_no']; //invoice create from
            $invStateNo=3;
            ...
            $origin_retailshop_no = $row['retailShop_no'];
            //if($row['retailShop_no'] != $retailShopNo)
            //    die(error_msg4());
            if($_SESSION['retail_no'] != 1)
                die(error_msg4());
            $createFrom = $_SESSION['retail_id'];
            $reason = $_GET['reason'];
    	
            $sql="INSERT INTO invoice (createDate,total,remark,invoiceType_no,staff_no,retailShop_no,invoiceState_no,createBy,createFrom,void_return_desc)
                                VALUES('$date','$total','$remark','$invoiceTypeNo',
                                       '$staffNo','$retailShopNo',$invStateNo,'$createBy','$createFrom','$reason')";
            echo $sql;
            $db->query($sql);
            echo $sql;
            ...
            if($goodsType==0){ //如果是accessories時做的動作
                ...
                $sql2="select stockIn_no, ava_bal,rec_qty from stockin where retailShop_no= $origin_retailshop_no
                       and acc_no = (select acc_no from accessories where barcode = '$product_no' or acc_id = '$product_no')
                       and rec_qty <> ava_bal
                       order by po_date desc, rec_date desc";
                $result2 = $db->query($sql2);
...
//						$cal_qty = $cal_qty-$maxQty;
            ...
            else if($goodsType==1){ //如果是電話時做的動作
                $sql="UPDATE phone SET `phoneState_no`=1 WHERE `IMEI`='$product_no';";
                $db->query($sql);
            }
	...
	case 'addInvoice':
		...
		if($goodsType==1){ //如果是電話時做的動作
			...
			$sql="select iprice, phonetype_id from phone left join phonetype on phone.phoneType_no = phonetype.phoneType_no where IMEI = '$poudNo'";
			$phone_res = $db->getrow($sql);
			$cost =  $phone_res['iprice'];
			$phonetype_id =  $phone_res['phonetype_id'];
                                 ...
                                 //   echo '<br> -qty '.$qty;
                                   ...
                                   // echo 'cost'.$cost;
                                    ...
                                    if($qty>0){
                                        ...
                                        $sql = "INSERT INTO invoicedetail (`product_no`,`description`,`qty`,`discount`,`price`,`invoice_no`,`modifyBy`,`goodsType`,`cost`,`commission_1`,`commission_2`,`ssa`,`ssa2`,`ssa_offer`)
                                        VALUES ('$poudNo','$description',$qty,'$discount','$price','$invoiceNo','$modifyBy',0,$cost,'$commission_1','$commission_2', $ssa, '$ssa2', '$phone_offer')";
                                        $db->query($sql);
	...
	case'getGdInfo':
        ...
        if ($result) {
            while ($row = $db->fetch_array($result)) {
                ...
                $discount = 0;
                $output_to_table = '<tr><td>'.$bcode.'</td>'.
                                       '<td>'.$accName.'</td>'.
                                       '<td>'.$price.'</td>'.
