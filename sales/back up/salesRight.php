...
<fieldset>
	...
	<table class="salesinfo">
		...
		<tr>
        	<td>店號</td><td id="shop_No" class="rightline">FTEC01</td><td>日期</td><td><div id="dd"></div></td>
        </tr>
        <tr>
        	<td>開單者</td>
...
$('#HIDD').load("../sales/salesGet.php?action=getbaseinfo&retail_id=FFF000");
					...
					$.ajax({
                    	...
                    	success: function(response) {
							...
							for ( i=0; i < itemArrLength; i++) {
									...
									//alert(itemArray[0]+" "+itemArray[i+1]+" "+itemArray[i+2]+" "+itemArray[i+3]+" "+itemArray[i+4]+" "+itemArray[i+5]);
...
var Tdisprice = 0;
		...
		$( "#discountForm" ).dialog({	
			...
			open: function () {
				...
				var disprice = $('#saltb tr:eq('+(delrow+1)+') td:eq(4)').html();
				Tdisprice = disprice;
				$('#disprice').val(disprice).select();
			},
			beforeOpen: function(){
			...
			buttons : {
				"確認": function() {
					...
					var tempprice = sDiscount;
			    	$('#temprice').val(tempprice);
				
					$('#saltb tr:eq('+(delrow+1)+') td:eq(4)').html(sDisprice);
					$('#saltb tr:eq('+(delrow+1)+') td:eq(5)').html(tempprice);
					
					if ((Tdisprice - $('#disprice').val())< 0)
						total = total - $('#disprice').val();
					else 
						total = total + (Tdisprice - $('#disprice').val());
					var deleteOne = (delrow*6);

					itemArray.splice((deleteOne+4),1,$('#disprice').val());
					itemArray.splice((deleteOne+5),1,tempprice);
					alert(itemArray);
...
<form>
	<fieldset>
    	<table cellpadding="5">
            ...
            <tr>
            	<td><label for="disprice" >折扣   -</label></td>
                <td>
            ...
            <tr>
            	<td><label for="temprice" class="">小計   </label></td>
                <td>
	                <input type="text" name="temprice" id="temprice" 
						class="text ui-widget-content ui-corner-all" maxlength="7" disabled="disabled"
						onclick="select()"/>
                </td>
            </tr>
