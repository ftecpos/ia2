...
//echo $sql;
	...
	case 'voidInvoice':
		...
		$sql2 = "UPDATE `3shop`.`invoice` SET `invoiceState_no`=2 WHERE `invoice_no`='$invNo';";	
		$db->query($sql2);
...
//						$qty = $qty-$maxQty;
			...
			if($goodsType==1){ //如果是電話時做的動作
				$sql="UPDATE `3shop`.`phone` SET `phoneState_no`=1 WHERE `IMEI`='$product_no';";
				$db->query($sql);
			}
...
//		if ($result) {
		if ($numOfRow > 0) {
			...
			echo  '<table rules="all" border="1" class="gidLeft" style="" >'.
				  '<tr>'.
				  '<td style="width: 130px" id="idn">IDN</td>'.
			...
			if($row2['invoiceState_no']==4)
				...
				echo  '<td style="width: 50px">折扣</td>'.
					  '<td style="width: 95px">小計</td><td style="width: 95px">種類</td></tr>';
				// $total = 0;
			$retuenQty=array();
			 ...
			 while ($row = $db->fetch_array($result)) {
				...
				if(($row2['invoiceState_no']==1||$row2['invoiceState_no']==4) && $row['pastIDV']==""){
					...
					while($row3 = $db->fetch_array($result3)){
						$retuenQty[]=$row3['qty'];
						$qqty=$qqty+$row3['qty'];
						echo  '<input type="button" value="退貨單編號 '.$row3['invoice_no'].' 退貨數量: '.$row3['qty'].'" class="finIncele" onclick="getInvoiceDetails('.$row3['invoice_no'].')"/>';
					}
					if($row['qty']!=$qqty)
				...
				echo	 '<td>'.$row['discount'].'</td>'.
						 '<td class="dig">'.$smallTotal.'</td>'.'<td>'.$row['goodsType'].'</td></tr>';
				//$total += $smallTotal;
			 }
...
//		$qty =(isset($_GET['returnQty']))? $_GET['returnQty'] :$row['qty'];
		...
		$goodsType=$row['goodsType'];
		$sql = "INSERT INTO invoicedetail (`product_no`,`qty`,`discount`,`price`,`invoice_no`,`modifyBy`,`pastIDV`,`description`,`goodsType`) VALUES ('$product_no','$qty','$discount','$price','$invoiceNo','$modifyBy','$pastIDV','$description','$goodsType')";
		$db->query($sql);
		echo $sql;
...
//						$qty = $qty-$maxQty;
	...
	case 'addInvoiceDetail':
		...
		if(!checkGoodsState($poudNo,$goodsType,$retailShopNo,$qty)){
			...
			$sql="DELETE FROM `3shop`.`invoicedetail` WHERE `invoice_no`='$invoiceNo';";
			echo $sql;
			$db->query($sql);
			
			$sql="DELETE FROM `3shop`.`invoice` WHERE `invoice_no`='$invoiceNo';";
			echo $sql;			
			$db->query($sql);
		...
		if($goodsType==1){ //如果是電話時做的動作
			$sql = "INSERT INTO invoicedetail (`product_no`,`description`,`qty`,`discount`,`price`,`invoice_no`,`modifyBy`,`goodsType`)
					VALUES ('$poudNo','$description',$qty,'$discount','$price','$invoiceNo','$modifyBy',1)";
			$db->query($sql);
			$db->query("UPDATE phone SET `phoneState_no`=2 WHERE`IMEI`='$poudNo'");
		}else 
		if($goodsType==0){ //如果是accessories時做的動作
			$sql = "INSERT INTO invoicedetail (`product_no`,`description`,`qty`,`discount`,`price`,`invoice_no`,`modifyBy`,`goodsType`)
				VALUES ('$poudNo','$description',$qty,'$discount','$price','$invoiceNo','$modifyBy',0)";
			$db->query($sql);
...
echo	"</script>";
	...
	case'getMobileInfo':
		...
		if ($inv_type==1){
			...
			$sql="SELECT phone_name,imei,manufacturer, sprice, oprice,phoneState_no
