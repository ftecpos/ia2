...
<?php
	//$PO_no = 7;
	$PO_no = $_GET["PO_no"];
	$PO_sql = mysql_query("select createDate, staff_id, supplier_id, supplierName, addr, phone from po, staff, supplier where po.supplier_no = supplier.supplier_no and po.staff_no = staff.staff_no and po_no ='".$PO_no."'");
	$PO_date = mysql_result($PO_sql,0,0);
	$staff_no = mysql_result($PO_sql,0,1);
	$sup_id = mysql_result($PO_sql,0,2);
	$sup_name = mysql_result($PO_sql,0,3);
	$addr = mysql_result($PO_sql,0,4);
	$tel = mysql_result($PO_sql,0,5);
?>
<?php
	$numHead = 'PO-';
...
<?php
	$ttc =0;
	$list_sql = mysql_query("select phonetype_no, acc_no, qty, cost, poDetail_no from podetail where po_no ='".$PO_no."'");	
