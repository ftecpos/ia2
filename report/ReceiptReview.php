...
<?php
	...
	$receipt_No = $_GET["receipt_No"];
        $sql = "select retail_id, addr, phone, createDate, createBy, total, remark, void_return_desc 
                from retailshop rs, invoice inv
                
                where rs.retailShop_no = inv.retailShop_no 
                
                and inv.invoice_no=".$receipt_No
                ;
               
    $result = $db->query($sql);
    $row = $db->fetch_array($result);
    if(!$row){ echo 'No record';  die; }
        $shop_no =$row['retail_id'];
        $address = $row['addr'];
        $tel = $row['phone'];
        $date = $row['createDate'];
        $staff_No = $row['createBy'];
        $total = $row['total'];
        $remark = $row['remark'];
        $reason = $row['void_return_desc'];
