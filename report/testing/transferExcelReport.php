...
if(isset($_GET['from']) && isset($_GET['to'])){
	...
	$endDay = $_GET['to'];
if($startDay != '' && $endDay != '') {	
	$sql .=" where transDate between '".$startDay." 00:00:00' and '".$endDay." 23:59:59' ";
} else if($startDay != ''){
	$startDay_s1 = $startDay." 00:00:00'";
	$endDay_s1 = date("Y-m-d")." 23:59:59'";
	$sql .=" where transDate between '".$startDay_s1." and '".$endDay_s1;
} else {
        $startDay_s1 = date("Y-m-d")." 00:00:00'";
	$endDay_s1 = date("Y-m-d")." 23:59:59'";
	$sql .=" where transDate between '".$startDay_s1." and '".$endDay_s1;
}}
...
while ($row1 = mysql_fetch_array($result)) {
	...
	$sql2 = "select transDate, transdetail.transfer_no, acc_id, transdetail.IMEI, accName, phonetype.manufacturer, phone_name, phonetype.color, phonetype_id, sum(trans_qty) as trans_qty, fr.retail_id as frid, tr.retail_id as trid, stateName, staff_id 
            FROM transfer 
            LEFT JOIN retailShop AS fr ON transfer.fromRetail_no = fr.retailShop_no 
            LEFT JOIN retailShop AS tr ON transfer.toRetail_no = tr.retailShop_no 
            LEFT JOIN transstate ON transfer.transState_no = transstate.transState_no 
            LEFT JOIN staff ON transfer.staff_no=staff.staff_no, transdetail 
            LEFT JOIN accessories ON transdetail.acc_no = accessories.acc_no
            LEFT JOIN  ( phone left join phonetype ON phone.phoneType_no = phonetype.phoneType_no) ON transdetail.IMEI = phone.IMEI 
            WHERE transfer.transfer_no = transdetail.transfer_no 
            AND transfer.transfer_no=".$trans_no;
