...
function addMobileToInvoice(phoneNo,shopNo,openssa){
    
    if(shopNo==shopno){
        _phoneNo = phoneNo;
        _shopNo = shopNo;
        if(openssa==1){
            $("#phone_ssa_dialog").dialog("open");
        } else {
            $.ajax({
                url:"../sales/salesGet.php?action=getMobileInfo",
                cache:false,
...
function openSupplierList(){
	$.ajax({
		url:"../sales/salesGet.php?action=getSupplierList",
		cache:false,
		dataType:'html',
		type:'GET',
		data:{ },
		error:function(xhr){alert('Ajax request Error!!!');},
		success:function(response){
			$('#listSupplierTop').html('<sapn>供應商名單</span>');
			$('#listSupplierBottom').html(response);
		}
	});
	
	$('#listSupplierForm').dialog('open');
}
function addSupplierToInvoice(supplier_id){
	if(supplier_id!=null){
		$('#inv_cust_no').html(supplier_id);
		$('#listSupplierForm').dialog('close');
	} else
		alert("Error");
}
