...
function addpo(costbyretail){
    ...
    $.ajax({
        url:"../maintain/getall.php?action=insertPo",
        cache: false,
        dataType: 'html',
        type:'GET',
        async: false,
        data:{	
            podetailm:arrayMobile,
            podetaila:arrayAcc,
            supplierno:supno,
            currTime:showtime(),
            page:pagenum1,
            potoretailno:costbyretail,
        },
        error: function(xhr){ alert('Ajax request Error!!!!!');	},
        success: function(response) {
        console.log(response);
                print_po_no = response;
                $('#printPO').dialog('open');
...
if(action == "getProduct" || action == "getStates") {
	...
	var productType1 = document.getElementById('productType');
        var retail_id = document.getElementById('costretailid').value;
...
if(action == "getProduct") {
	xmlhttp.open("GET","../maintain/getall.php?action=getProdectforPo&page="+pagenum+"&code="+code+"&name="+name+"&branch="+branch+"&type="+type+"&ed=getP&retailid="+retail_id,true);
} else if(action == "getSupplier") {
	xmlhttp.open("GET","../maintain/getall.php?action=getSupplier&supid="+supplierid,true);
} else if(action == "insertPo") {
	xmlhttp.open("GET","../maintain/getall.php?action=insertPo&podetailm="+arrayMobile+
                "&podetaila="+arrayAcc+"&supplierno="+supno+"&currTime="+showtime()+
                "&page="+pagenum1,true);
